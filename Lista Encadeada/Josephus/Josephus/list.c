...
void insertFirst(LIST *ptrList, char *ptrName)
{
    ...
    ptrNoAux = createNode(); // Cria um n�

    ptrNoAux->nome = ptrName; // seta um dado no n�

    if(isEmpty(ptrList)){// verifica se a lista ta vazia, o ponteiro para o pr�ximo n� vai apontar para ele mesmo.
...
int onthelist(LIST* ptrList, NODE* ptrNode)
{
    ...
    if(!strcmp(listAux.first->nome, ptrNode->nome)) // Verifica se � o node procurado
        return 1;

    if(listAux.first->next == ptrList->first) // Condi��o de parada, verifica se o pr�ximo node � igual ao primeiro
        return 0;

    listAux.first = ptrList->first->next; // faz aux.first apontar para o pr�ximo node da lista

    onthelist(&listAux, ptrNode); // faz chamada recursiva da fun��o
}
...
char *deleteAfter(LIST *ptrList, NODE *ptrNode)
{
    ...
    nodeAux = ptrNode->next; // guarda a referencia para o prox�mo n�

    if(nodeAux == ptrList->first)
        ptrList->first = nodeAux->next;

    ptrNome = nodeAux->nome; // guarda o dado do n� a ser excluido
    ptrNode->next = nodeAux->next; // faz o n� atual a apontar o proximo do n� que vai ser excluido

    --(ptrList->sizeList);

    free(nodeAux); // libera o n� que vai ser excluido
...
NODE *roletaRomana(LIST *ptrLista)
{
    ...
    while(ptrLista->sizeList > 1)
    {
        ...
        if(!isEmpty(ptrLista))  //se a lista n�o tiver vazia elimina pessoa
